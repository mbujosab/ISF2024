...
\begin{tikzpicture}
  \begin{axis}[width=10cm,  height=4cm, ytick pos=left, date coordinates in=x,xticklabel={\tiny \year},x tick label style={rotate=45,anchor=north east},xmin=1855-06-01, xmax=2015-12-01,xmajorgrids=true,grid style=dashed,axis on top, lustro ticks={1},
    ...
    % \node[below] at (axis description cs: 1991-06-12, 0) {Pinatubo};
    \pgfplotsset{
      after end axis/.code={
        \node[above] at (axis cs: 1883-08-26,  1.5) {\footnotesize  Krakatoa };
        \node[below] at (axis cs: 1883-08-26,  1.5) {\footnotesize (Aug-1883)};
        \node[above] at (axis cs: 1991-06-21, -0.7) {\footnotesize  Pinatubo };
        \node[below] at (axis cs: 1991-06-21, -0.7) {\footnotesize (Jun-1991)};
      }
    }
  \end{axis}
\end{tikzpicture}
