...
%\newcommand\concatSV[2]{\ensuremath{#1{\sim}#2}\xspace}
\newcommand\concatSV[2]{\ensuremath{#1{\|}#2}\xspace}
%\renewcommand\concatSV[2]{\ensuremath{#1{\sqcup}#2}\xspace}
\renewcommand\concatSV[2]{\ensuremath{#1{\sim}#2}\xspace}
%\renewcommand\concatSV[2]{\ensuremath{#1{\mathbin{\mathaccent\cdot\cup}}#2}\xspace}
%\newcommand\BigconcatSV[2]{\ensuremath{#1{\Big\|}#2}\xspace}
...
\newcommand{\vectPE} [2][{}]{\ensuremath{\left({\vec{\MakeLowercase{#2}}}_{#1}\right)\xspace}}
...
\newcommand{\MVect}[2][{}]{\ensuremath{\big[{{{\boldsymbol{\MakeLowercase{#2}}}_{#1}}}\vphantom{\Big(}\big]  \xspace}}

\newcommand{\MVectT}[2][{}]{\ensuremath{\big[{{{\boldsymbol{\MakeLowercase{#2}}}_{#1}}}\vphantom{\Big(}\big]^\T \xspace}}
...
\renewcommand{\VectT}[2][{}]{\ensuremath{\big[{{{\boldsymbol{\MakeLowercase{#2}}}_{#1}}}\vphantom{\Big(}\big]^{\!\T}\xspace}} %% OJO!
...
\RequirePackage{mathtools}   %% para índices a la izquierda \prescript
...
\newcommand{\elemLRP}[3]{\ensuremath{\prescript{\phantom{\T}}{\getitemL{#2}}{\big(#1\big)}^{\phantom{\T}}_{\getitemR{#3}}}\xspace}
...
\newcommand{\Cof}[3]{\ensuremath{\cof_{{#2}{#3}}\Big(\Mat{#1}\Big)}\xspace}
\newcommand{\Coff}[3]{\ensuremath{\cof_{{#2}{#3}}(\Mat{#1})}\xspace}


\newcommand{\OEg}{\ensuremath{\pmb{\tau}}\xspace}
\newcommand{\InvTrEE}{\ensuremath{{\pmb{\tau}}^{\minus 1}}\xspace}
\newcommand{\ETrEE}{\ensuremath{{esp(\pmb{\tau}})}\xspace}
\newcommand{\EInvTrEE}{\ensuremath{esp({\pmb{\tau}}^{\minus 1})}\xspace}
%\newcommand{\TrE}[1]{\delimitershortfall=-1pt\ensuremath{\underset{\left[{#1}\right]}{\OEg}}\xspace}
\newcommand{\TrE}[1]{\ensuremath{\underset{\left[{#1}\right]}{\OEg}}\xspace}
...
%\newcommand{\pe}[2]{ \pmb{#1} \rightleftharpoons \pmb{#2} }  % intercambio
%\newcommand{\pr}[2]{{#2} \cdot {\pmb{#1}}}                   % producto
%\newcommand{\su}[3]{{\pmb{#1}}{#3}\cdot{\pmb{#2}}}           % suma


%%% TRANSFORMACIONES ELEMENTALES GENÉRICAS
%% suceción de transformaciones elementales genéricas desde #1 hasta #2
\newcommand{\SSTEC}[3]{  \ensuremath{                    {#3}_{\OEg_{#1}\cdots\OEg_{#2}}^{\phantom{\T}} }\xspace }
\newcommand{\SSTEF}[3]{  \ensuremath{ \prescript{\phantom{\T}}{\OEg_{#1}\cdots\OEg_{#2}}{#3}            }\xspace }
\newcommand{\SSTEFC}[3]{ \ensuremath{ \prescript{\phantom{\T}}{\OEg_{#2}\cdots\OEg_{#1}}{#3}_{\OEg_{#1}\cdots\OEg_{#2}}^{\phantom{\T}} }\xspace }


%\usepackage{pgffor}
%\newcommand{\VSTEC}[1]{ \ensuremath{%
%  \underset{   
%   \begin{subarray}{c}
%    \foreach \tf in {#1} { \left[  \tf \right] \\}
%    \end{subarray}}{\OEg}}
%  }
  

\newcommand{\TrC}[2][]{%                       %% Ejemplo: \TrC[ \TrE{\su{i}{j}{+5}} \TrE{\pr{j}{-7}} ]{\Mat{A}}
...
\newcommand{\TrF}[2][]{%                       %% Ejemplo: \TrF[ \TrE{\su{i}{j}{+5}} \TrE{\pr{j}{-7}} ]{\Mat{A}}
  \ifthenelse{\isempty{#1}}%
    { \SSTEF{1}{k}{#2} }                                   % if #1 is empty
    { \ensuremath{ \prescript{\phantom{\T}}{#1}{#2} } } 
    \xspace
}

\newcommand{\TrFC}[3]{%   Por los dos lados!   %% Ejemplo: \TrFC{\TrE{\su{i}{j}{-5}}}{\TrE{\pr{j}{-7}}}{\Mat{A}}
    \ensuremath{\prescript{\phantom{\T}}{#1}{{#3}}^{\phantom{\T}}_{#2}} 
    \xspace                                          
}   

\newcommand{\TrCP}[2][]{\ensuremath{\TrC[#1]{\left(#2\right)}\xspace}}
\newcommand{\TrFP}[2][]{\ensuremath{\TrF[#1]{\left(#2\right)}\xspace}}
\newcommand{\TrCPE}[2][]{\ensuremath{\left(\TrC[#1]{#2}\right)\xspace}}
\newcommand{\TrFPE}[2][]{\ensuremath{\left(\TrF[#1]{#2}\right)\xspace}}

\newcommand{\TEC}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrC[\OEg]{#2}}                 % if #1 is empty   A_tau
    {\TrC[\OEg_{#1}]{#2}}            %                  A_{tau_{#1}}
    \xspace
}

\newcommand{\TEF}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrF[\OEg]{#2}}                 % if #1 is empty
    {\TrF[\OEg_{#1}]{#2}}
    \xspace
}
...
\newcommand{\TECE}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrC[esp(\OEg)]{#2}}                 % if #1 is empty   A_tau
    {\TrC[esp(\OEg_{#1})]{#2}}            %                  A_{tau_{#1}}
    \xspace
}

\newcommand{\TEFE}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrF[esp(\OEg)]{#2}}                 % if #1 is empty
    {\TrF[esp(\OEg_{#1})]{#2}}
    \xspace
}
...
\newcommand{\InvTEC}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrC[\InvTrEE]{#2}}                 % if #1 is empty   A_tau
    {\TrC[\InvTrEE_{#1}]{#2}}            %                  A_{tau_{#1}}
    \xspace
}

\newcommand{\InvTEF}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrF[\InvTrEE]{#2}}                 % if #1 is empty
    {\TrF[\InvTrEE_{#1}]{#2}}
    \xspace
}
...
\newcommand{\InvTECE}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrC[esp(\InvTrEE)]{#2}}                 % if #1 is empty   A_tau
    {\TrC[esp(\InvTrEE_{#1})]{#2}}            %                  A_{tau_{#1}}
    \xspace
}

\newcommand{\InvTEFE}[2][]{%
  \ifthenelse{\isempty{#1}}%
    {\TrF[esp(\InvTrEE)]{#2}}                 % if #1 is empty
    {\TrF[esp(\InvTrEE_{#1})]{#2}}
    \xspace
}

\newcommand{\TECP}[2][]{\ensuremath{\TEC[#1]{\left(#2\right)}\xspace}}
\newcommand{\TEFP}[2][]{\ensuremath{\TEF[#1]{\left(#2\right)}\xspace}}
\newcommand{\TECPE}[2][]{\ensuremath{\left(\TEC[#1]{#2}\right)\xspace}}
\newcommand{\TEFPE}[2][]{\ensuremath{\left(\TEF[#1]{#2}\right)\xspace}}

\newcommand{\TECEP}[2][]{\ensuremath{\TECE[#1]{\left(#2\right)}\xspace}}
\newcommand{\TEFEP}[2][]{\ensuremath{\TEFE[#1]{\left(#2\right)}\xspace}}
\newcommand{\TECEPE}[2][]{\ensuremath{\left(\TECE[#1]{#2}\right)\xspace}}
\newcommand{\TEFEPE}[2][]{\ensuremath{\left(\TEFE[#1]{#2}\right)\xspace}}

\newcommand{\InvTECP}[2][]{\ensuremath{\InvTEC[#1]{\left(#2\right)}\xspace}}
\newcommand{\InvTEFP}[2][]{\ensuremath{\InvTEF[#1]{\left(#2\right)}\xspace}}
\newcommand{\InvTECPE}[2][]{\ensuremath{\left(\InvTEC[#1]{#2}\right)\xspace}}
\newcommand{\InvTEFPE}[2][]{\ensuremath{\left(\InvTEF[#1]{#2}\right)\xspace}}

\newcommand{\InvTECEP}[2][]{\ensuremath{\InvTECE[#1]{\left(#2\right)}\xspace}}
\newcommand{\InvTEFEP}[2][]{\ensuremath{\InvTEFE[#1]{\left(#2\right)}\xspace}}
\newcommand{\InvTECEPE}[2][]{\ensuremath{\left(\InvTECE[#1]{#2}\right)\xspace}}
\newcommand{\InvTEFEPE}[2][]{\ensuremath{\left(\InvTEFE[#1]{#2}\right)\xspace}}

%% matricial

% SUMAS
\newcommand{\TESC}[4]{\TrC[\!\TrE{\su{#1}{#2}{#3}}]{#4}}
\newcommand{\TESF}[4]{\TrF[\TrE{\su{#1}{#2}{#3}}\!]{#4}}
...
\newcommand{\TESCPE}[4]{\ensuremath{\left({{#4}}^{\phantom{\big(}}_{\!\!\!\TrE{\su{#1}{#2}{#3}}}\right)}\xspace}
\newcommand{\TESFPE}[4]{\ensuremath{\right(\prescript{\phantom{\big(}}{\TrE{\su{#1}{#2}{#3}}\!\!}{{#4}}\right)}\xspace}

% PRODUCTOS
\newcommand{\TEPC}[3]{\TrC[\!\TrE{\pr{#1}{#2}}]{#3}}
\newcommand{\TEPF}[3]{\TrF[\TrE{\pr{#1}{#2}}\!]{#3}}
% con paréntesis
...
\newcommand{\TEPCPE}[3]{\ensuremath{\left({{#3}}^{\phantom{\big(}}_{\!\!\!\TrE{\pr{#1}{#2}}}\right)}\xspace}
\newcommand{\TEPFPE}[3]{\ensuremath{\left(\prescript{\phantom{\big(}}{\TrE{\pr{#1}{#2}}\!\!}{{#3}}\right)}\xspace}

% INTERCAMBIOS
\newcommand{\TEIF}[3]{\TrF[\TrE{\pe{#1}{#2}}\!]{#3}}
\newcommand{\TEIC}[3]{\TrC[\!\TrE{\pe{#1}{#2}}]{#3}}
% con paréntesis
\newcommand{\TEIFP}[3]{\ensuremath{\prescript{\phantom{\big(}}{\TrE{\pe{#1}{#2}}\!\!}{\left({#3}\right)}}\xspace}
\newcommand{\TEICPE}[3]{\ensuremath{\Big({{#3}}^{\phantom{\big(}}_{\!\!\!\TrE{\pe{#1}{#2}}}\Big)}\xspace}
\newcommand{\TEIFPE}[3]{\ensuremath{\Big(\prescript{\phantom{\big(}}{\TrE{\pe{#1}{#2}}\!\!}{{#3}}\Big)}\xspace} 
...
%\newcommand{\EI}[3]{\ensuremath{\boldsymbol{\svec{E}}_{\boldsymbol{{[#1,#2]}}}^{\left(#3\right)}}}
%\newcommand{\EII}[2]{\ensuremath{\boldsymbol{\svec{E}}_{\boldsymbol{{[#1,#1]}}}^{\left(#2\right)}}}
...
\newcommand{\EI}[3]{\TESC{#1}{#2}{#3}{\Mat{I}}}
\newcommand{\EII}[2]{\TEPC{#1}{#2}{\Mat{I}}}
...
\newcommand{\Mint}[2]{\ensuremath{\TEIC{#1}{#2}{\Mat{I}}}\xspace}

\newcommand{\PF}[1]{\TrF[\TrE{\circlearrowleft}]{\Mat{#1}}}
\newcommand{\PC}[1]{\TrC[\TrE{\circlearrowleft}]{\Mat{#1}}}
...
\newcommand{\MP}[1][]{%
  \ifthenelse{\isempty{#1}}%
    {\ensuremath{{\TrC[\TrE{\circlearrowleft}]{\Mat{I}}}}}  % if #1 is empty
    {\ensuremath{{\TrC[\TrE{\circlearrowleft}]{\Mat{I}}}}}
    \xspace
}
...
\newcommand{\elemLLLRRR}[3]{\ensuremath{\textrm{elem}_{#2#3}{\left(#1\right)}}\xspace}% con texto "elem"
...
\newcommand{\eleMT}[3]{\ensuremath{\elemLRP{\Mat{#1}^\T}{#2}{#3}}\xspace}    % con subindices
\newcommand{\eleMP}[3]{\ensuremath{\elemLRP{\Mat{#1}}{#2}{#3}}\xspace}          % con subindices
\newcommand{\eleMTP}[3]{\ensuremath{\elemLRP{\Mat{#1}^\T}{#2}{#3}}\xspace}    % con subindices
...
\newcommand{\VectCCC}[2][{}] {\ensuremath{\elemRRR{\left(\Mat{#2}\right)}{#1}}\xspace}   % con texto "col"
\newcommand{\VectFFF}[2][{}] {\ensuremath{\elemLLL{\left(\Mat{#2}\right)}{#1}}\xspace}   % con texto "fila" o "row"

%SELECCIÓNA de FILAS y COlUMNAS DE UNA MATRIZ TRANSPUESTA PARA GENERAR UN VECTOR DE Rn
\newcommand{\VectTC}[2][{}]  {\ensuremath{\elemRP{\MatT{#2}}{#1}}\xspace}  % con subindices
\newcommand{\VectTF}[2][{}]  {\ensuremath{\elemLP{\MatT{#2}}{#1}}\xspace}  % con subindices
\newcommand{\VectTCP}[2][{}]  {\ensuremath{\elemRP{\MatT{#2}}{#1}}\xspace}  % con subindices
\newcommand{\VectTFP}[2][{}]  {\ensuremath{\elemLP{\MatT{#2}}{#1}}\xspace}  % con subindices
...
\newcommand{\MatTP}[2][{}]{\ensuremath{{{(\mat{#2})}^{\T}_{#1}}}\xspace} 
\newcommand{\MatTPE}[2][{}]{\ensuremath{{\big({\mat{#2}}^{\T}_{#1}\big)}}\xspace} 
...
\renewcommand{\ViTWj}[4]{\ensuremath{{\MVectT[#2]{#1}}{\MVect[#4]{#3}}}\xspace}
\renewcommand{\ViTVj}[3]{\ViTWj{#1}{#2}{#1}{#3}}
...
\renewcommand{\ViWjT}[4]{\ensuremath{\MVect[#2]{#1}{\VectT[#4]{#3}}}\xspace}
\renewcommand{\ViVjT}[3]{\ensuremath{\ViWjT{#1}{#2}{#1}{#3}}\xspace}
\renewcommand{\ViViT}[2][{}]{\ensuremath{\MVect[#1]{#2}{\VectT[#1]{#2}}}\xspace}
...
\newcommand{\InvMatPE}[2][{}]{\ensuremath{\left({\mat{#2}_{#1}^{\minus1}}\right)}\xspace} 
%% Matriz #1 traspuesta
%\newcommand{\MatT}[1]{\ensuremath{{\Mat{#1}\!}^{\T}\,}\xspace} 
